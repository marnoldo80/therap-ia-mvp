...
export default function PatientPage() {
  ...
  async function sendAccountInvite() {
    setMsg(null);
    setErr(null);
    if (!email) return setErr("Inserisci l'email del paziente e salva prima.");
    try {
      const res = await fetch("/api/invite-patient", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, patientId: id }),
      });
      const j = await res.json();
      if (!res.ok) throw new Error(j?.error || "Invio invito fallito.");
      setMsg("Invito account inviato al paziente via email.");
    } catch (e: any) {
      setErr(e?.message || "Errore invio invito");
    }
  }
  ...
  async function sendInviteEmail() {
    ...
    try {
      const res = await fetch("/api/send-gad7-invite", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ to: email, name: displayName || "Paziente", url }),
      });
      const j = await res.json();
      if (!res.ok) throw new Error(j?.error || "Invio fallito");
      setMsg("Invito GAD-7 inviato via email.");
    } catch (e: any) {
  ...
  function openWhatsApp() {
    if (!inviteToken) return setErr("Nessun link generato.");
    const url = `${window.location.origin}/q/gad7/${inviteToken}`;
    const text = encodeURIComponent(`${displayName || ""}, compila il GAD-7 qui: ${url}`);
        ...
        {rows.map((r, i) => (
          <div key={i} className="rounded border px-3 py-2 flex items-center justify-between">
            <div>
              ...
              <span className="text-xs text-gray-600 border rounded px-2 py-0.5 ml-2">{r.severity}</span>
            </div>
