...
export default function Gad7Page({ params }: { params: { id: string } }) {
  ...
  const total = useMemo(
    () => answers.reduce((a, b) => a + (Number.isFinite(b) ? b : 0), 0),
    [answers]
  );
  ...
  async function sendResultsByEmail() {
    if (!patient?.email) {
      setErr("Email paziente mancante.");
      return;
    }
    if (!savedRowId) {
      setErr("Salva prima il risultato.");
      return;
    }
    setSending(true);
    setErr(null);
  ...
  return (
    <main className="mx-auto max-w-3xl p-6">
      <div className="mb-6">
        <button
          onClick={() => router.push(`/app/therapist/pazienti/${patientId}`)}
      ...
      {/* messaggi */}
      {err && (
        <div className="mb-4 rounded-md border border-red-300 bg-red-50 px-4 py-2 text-red-700">
          {err}
        </div>
      )}
      {msg && (
        <div className="mb-4 rounded-md border border-emerald-300 bg-emerald-50 px-4 py-2 text-emerald-700">
          {msg}
        </div>
      )}
      ...
      <div className="mt-6 rounded-xl border p-4">
        ...
        <p className="text-3xl font-semibold">
          {total} <span className="text-base font-normal">/ 21</span>
        </p>
        <p className="mt-1 text-gray-700">
          Gravità: <b>{severity}</b>
        </p>
      </div>
