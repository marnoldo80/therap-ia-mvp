...
La sottoscritta dott.ssa ${therapist.full_name}, iscritta all'Ordine degli Psicologi del Veneto con il n. ${therapist.registration_number}, orientamento ${therapist.therapeutic_orientation}, codice fiscale ${therapist.tax_code}, partita IVA ${therapist.vat_number}, prima di svolgere la propria opera professionale a favore del sig./della sig.ra ${patient.display_name}, lo/la informa di quanto segue:
...
- le prestazioni verranno rese presso lo studio sito in ${therapistFullAddress}, o in modalità online;
...
- la durata dell'intervento è di ${patient.session_duration_individual || 45} minuti per la terapia individuale, ${patient.session_duration_couple || 60} minuti per la terapia di coppia, ${patient.session_duration_family || 75} minuti per la consulenza familiare;
- il compenso per ciascuna seduta è di Euro ${patient.rate_individual || 90} per sedute individuali, Euro ${patient.rate_couple || 130} per sedute di coppia, Euro ${patient.rate_family || 150} per sedute familiari, oltre al 2% ENPAP e agli eventuali oneri di legge;
...
Il/la sig./sig.ra ${patient.display_name}, nato/a a ${patient.birth_place || '_____'} il ${patient.birth_date ? new Date(patient.birth_date).toLocaleDateString('it-IT') : '_____'}, codice fiscale ${patient.fiscal_code || '_____'}, residente a ${patientFullAddress}, avendo ricevuto le informazioni di cui sopra e preso visione della polizza assicurativa n. ${therapist.insurance_policy}, dichiara:
- di aver compreso i termini dell'intervento e di accettare quanto concordato con la dott.ssa ${therapist.full_name};
...
La dott.ssa ${therapist.full_name}, in qualità di titolare del trattamento, informa che i dati personali, anche particolari o sensibili, saranno trattati nel pieno rispetto dei diritti, delle libertà fondamentali e della dignità del paziente, nel rispetto della riservatezza e delle normative vigenti.
